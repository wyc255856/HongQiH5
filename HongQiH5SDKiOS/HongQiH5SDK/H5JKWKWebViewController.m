...
#import "H5JKEventHandler.h"
#import "JKEventHandler+H5Car.h"
#import "H5ShareManager.h"
#import "H5LoadFailedAlertView.h"
#import "H5LoadingProgressView.h"
#import "H5CAR_SSZipArchive.h"
#import "H5CAR_AFHTTPRequestOperation.h"




@interface H5JKWKWebViewController ()<WKNavigationDelegate, WKUIDelegate, LoadFailedAlertViewDelegate>
...
@implementation H5JKWKWebViewController
...
- (void)viewDidLoad {
    ...
    [H5ShareManager shareInstance].wkWebVC = self;
...
- (void)configureWKWebview{
    ...
    WKUserScript *usrScript = [[WKUserScript alloc] initWithSource:[H5JKEventHandler shareInstance].handlerJS injectionTime:WKUserScriptInjectionTimeAtDocumentEnd forMainFrameOnly:YES];
    ...
    [config.userContentController addScriptMessageHandler:[H5JKEventHandler shareInstance] name:EventHandler];
    ...
    if([userDefaults objectForKey:@"webviewH5LoadMode"]){
        ...
        [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/?upLoad=%@",[H5ShareManager shareInstance].wkWebVC.url,sUpLoad]]]];
    }
    [self.view addSubview:_webView];
    
    [H5JKEventHandler getInject:_webView];
...
- (void)showLoadFailedAlert{
    H5LoadFailedAlertView *alertView = [[H5LoadFailedAlertView alloc]initWithTitle:@"" message:@"" delegate:self leftButtonTitle:@"确定" rightButtonTitle:@"取消"];
    [alertView show];
}
...
// 输入框弹窗的button点击时回调
- (void)declareAbnormalAlertView:(H5LoadFailedAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
...
- (void)releaseZipFilesWithUnzipFileAtPath:(NSString *)zipPath Destination:(NSString *)unzipPath{
    NSError *error;
    if ([H5CAR_SSZipArchive unzipFileAtPath:zipPath toDestination:unzipPath overwrite:YES password:nil error:&error delegate:self]) {
//        NSLog(@"success");
...
//    NSLog(@"filePath: %@", filePath);
    ...
    H5LoadingProgressView *alertView = [[H5LoadingProgressView alloc] initProgressView];
    [alertView show];
...
//        NSLog(@"%f",progress);
    ...
    } failure:^(H5CAR_AFHTTPRequestOperation *operation, NSError *error) {
